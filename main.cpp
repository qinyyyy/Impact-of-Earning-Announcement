...
void plot_caar(map<string, vector<Vector>> calculation_results) {
    ...
    gnuplotPipe = _popen("C:\ProgramData\Microsoft\Windows\Start Menu\Programs\gnuplot\gnuplot 5.4 patchlevel 2","w");
    if (gnuplotPipe) {
        ...
        case 1:
            ...
            for (int i = 0; i < (int)curSymbols.size(); i++) {
                ...
                if ((int)price_map[curSymbol].size() < 2 * N + 1) {
                    ...
                    ARtable[curSymbol] = stockReturn - benchmarkReturn;//overide
                }
            }
